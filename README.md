...
![Panocode 首页首屏](public/screenshots/home-hero.png)

突出产品定位、双入口和核心价值点，适合放在 README 顶部帮助读者快速理解产品用途。

### 分析工作台

![Panocode 分析工作台](public/screenshots/analysis-workspace.png)

展示文件树、源码查看、仓库摘要、分析日志与调用全景的协同工作流。

### 在线 Demo

如果后续提供公开访问地址，可以在这里补充 Demo 链接。
...
1. 在首页切换到本地项目
2. 如果 Panocode 运行在你的本机浏览器里，可以直接输入本地路径
3. 如果 Panocode 部署在远端服务器上，可以使用支持 File System Access API 的浏览器点击“选择文件夹”授权访问
4. 如果浏览器不支持文件夹授权，或你希望更稳定地跨环境使用，也可以直接上传 ZIP 压缩包
5. 首页本地项目区域支持直接拖拽 ZIP 上传；上传后的 ZIP 会缓存到当前浏览器，刷新页面后仍可恢复
6. 点击 Analyze
7. 进入分析页后查看文件树、代码内容和调用图结果
...
- 本地项目分析支持三种入口：本机路径、本地文件夹授权、ZIP 上传；其中远端部署不能直接访问访问者电脑上的路径
...
浏览器文件夹选择依赖 File System Access API，并非所有浏览器都完整支持。

如果你访问的是远端部署版本，直接输入本地路径并不会读取你电脑上的目录，而是读取服务器磁盘。这种情况下只能使用“选择文件夹”授权浏览器访问，或者把 Panocode 跑在本机再用路径模式。

如果浏览器不支持 File System Access API，或者你需要把项目分享给其他人远端分析，直接上传 ZIP 压缩包通常是更稳定的方案。

当前版本会把上传后的 ZIP 缓存到浏览器 IndexedDB 中，因此在同一浏览器里刷新页面或带链接回到分析页时，通常可以直接恢复。
